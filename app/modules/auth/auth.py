...
def login():
    if request.method == 'POST':
        ...
        cursor.execute(
            'SELECT * FROM usuarios WHERE usuario=%s AND CONTRASENIA=%s', (usuario, contrasenia))
        account = cursor.fetchone()
        if account:
            return redirect(url_for('auth.profile'))
        else:
            return render_template('auth/login.html', login_error='Credenciales incorrectas o usuario no encontrado')
